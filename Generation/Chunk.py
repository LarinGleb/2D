...
class Chunk:
    ...
    def Generate(self, screen, heightmap):
        ...
        for i in range(1, GenerationSettings.BLOCKS_IN_CHUNK + 1):
            ...
            for i in range(1, currentY):
                block = Block.Block(
                    currentX,
                    (currentY + i) * GenerationSettings.SIZEBLOCK,
                    GetSprite(i),
                    screen,
                )
    ...
    def CheckCoordsInside(self, position):
        x, y = position
        return (
            x > self.positionStart[0] * GenerationSettings.SIZEBLOCK
            and x
            < (self.positionStart[0] + GenerationSettings.BLOCKS_IN_CHUNK)
    ...
    def GenerateNext(self, heightmap):
        if self.player.directional == 1:
            if (
                ...
                > (
                    self.player.chunk.positionStart[0]
                    + GenerationSettings.BLOCKS_IN_CHUNK // 2
                )
                * GenerationSettings.SIZEBLOCK
                and not self.GeneratedRight
            ):
                ...
                chunk = Chunk(
                    [
                        oldPosition[0]
                        + self.player.directional * GenerationSettings.BLOCKS_IN_CHUNK,
                        oldPosition[1],
                    ],
                    self.player,
                    self.screen,
            ...
            if (
                ...
                < (
                    self.player.chunk.positionStart[0]
                    + GenerationSettings.BLOCKS_IN_CHUNK
                )
                * GenerationSettings.SIZEBLOCK
                and not self.GeneratedLeft
            ):
                oldPosition = self.player.chunk.positionStart
