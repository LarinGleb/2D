...
def Path(save = False):
    ...
    if save:
        fileName = tkinter.filedialog.asksaveasfile(parent=tkTop, defaultextension=".sv", filetypes=[("Save", "*.sv")]).name
    else:
        fileName = tkinter.filedialog.askopenfilename(parent=tkTop, defaultextension=".sv", filetypes=[("Save", "*.sv")])
    tkTop.destroy()
    PATH = fileName
    ...
    if tf:
        ...
        for i in range(n):
            for j in range(y):
                ...
                image.set_at((j, i), (int(r + k // 50), int(g + k // 50), int(b + k // 50)))
            if k < 1:
                break
            else:
                k -= 1
...
def main(screen, playerConfig = [[Settings.WIDTH // 2, Settings.HEIGHT // 2], [2,8]], seed = GenerationSettings.SEED):
    ...
    startChunk = Generation.Chunk.Chunk([playerConfig[1][0], playerConfig[1][1]], player, screen)
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    Changes.FixChange.SaveChanges(PATH, playerPos, player.inventory, seed, player.chunk)
                    pygame.quit() 
                    sys.exit()
                                ...
                                if event.button == 3 and player.chooseCell:
                                    ...
                                    for block in chunk.listBlocks:
                                        ...
                                        if block.Position[0] == blockCoords[0] and block.Position[1] == blockCoords[1]:
                                            ...
                                            change = Changes.Change.Change("break", block)
                                            Changes.FixChange.Append(change)
...
if __name__ == "__main__":
    ...
    running = True
    NewWorld = Button.Button([Settings.WIDTH // 2, Settings.HEIGHT // 2], (300, 100), "Create new world", SaveNewWorld)
    OpenWorld = Button.Button([Settings.WIDTH // 2, Settings.HEIGHT // 2 + 150], (300, 100), "Open world", LoadWorld)
